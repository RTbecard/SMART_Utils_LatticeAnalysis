...
## Load patrol data
patrols <- readOGR(dsn="/home/james/Downloads/SMART Patrols", layer="Patrol",stringsAsFactors = options(stringsAsFactors = T))
#head(patrols@data)
#spplot(patrols,zcol = 'Patrol_ID')

## Load Protected Area Basemap
map <- readOGR(dsn="/home/james/Downloads/SMART Patrols", layer="PATRL",stringsAsFactors = options(stringsAsFactors = T))
# Convert names to factors
#head(map@data)
#spplot(map,zcol = 'name',scales = list(draw = T))
...
###################   TESTING   #######################
#######################################################
output <- LatticedAnalysis(patrols,map,0.02)
###################
### Plot Resuts ###
###################
spplot(output,zcol = 'distance.patrolled',main = 'Distance Patrolled per Cell',scales = list(draw = T))
spplot(output,zcol = 'Patrol.Visits',scales = list(draw = T),main = 'Visits by patrols')
spplot(output,zcol = 'Percent.coverage',scales = list(draw = T),main = 'Percent Coverage per Region')
